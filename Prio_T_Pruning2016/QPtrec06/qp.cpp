...
QueryProcessing::QueryProcessing(termsCache& cache) : qn(0), Cache(cache), trecReader(0) {
	...
	p.add(" Total and average time building DT: ",CONSTS::STEP1);
	p.add(" Total and average time processing docs: ",CONSTS::STEP2);
	p.add(" Total and average time doing lookups: ",CONSTS::STEP3);
...
void QueryProcessing::operator()(const char* queryLog, const int buckets, const int limit, const int topk, const int layer)	{
	...
	profilerC& p = profilerC::getInstance();
	profilerC& ptest = profilerC::getInstance();
	...
	while( qn++ < 1) {
		...
		// ############doc pori###########
		// cout << "RM initialized" <<endl;
		RetManager *reter = new RetManager(word_l, lex, Cache, termsInCache);
		// cout << "RM initialized done" <<endl;
		p.start(CONSTS::ALLQS);
		total_docs += reter->retrieval(topk, pages, ptest);	
		total_eval += reter->getEvalCounter();
		p.end(CONSTS::ALLQS); 
