	...
	for(i=0;i<num;i++)
	{
		...
		if(it != lex.end()){
			...
			if(r[i].PR ==NULL){
				...
				cout<<"Could not find the term "<<word_l[i]<<endl;
			}
...
uint RetManager::retrieval(unsigned retNum, unsigned* pages, profilerC& p)
{
	...
	// cout << "BM initialize" << endl;
	BM = new BlockManager(r,num,retNum);
	// cout << "BM initialize done" << endl;
	// p.end(CONSTS::STEP1);
	...
	{
		BM->putDoc(curDoc,r);//DT->putDoc(curDoc,r);
		curDoc = findNextDoc();
	...
	BlockNode ** blockList = BM->getBlockList();//DecisionTreeNode **blockList = DT->getBlockList();
	vector<pair<unsigned,unsigned*> >::iterator it;
	// p.start(CONSTS::STEP3);
	for(i=0;i<BM->getBlockNum();i++)//for(i=0;i<DT->getBlockNum();i++)
	{	
		// cout << "i: "<< i << endl;
