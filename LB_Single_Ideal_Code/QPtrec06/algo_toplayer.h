...
private:
  ...
  /*lower bound to store the second layer threshold*/
  vector<float> lowerbound_scores;
...
//         	so the size of cutoff vector must be depths.size() + depths_pairs.size().
 inline void onlineGreedyDepthSelectionAlgorithmUnify(const uint space_budget,
                                          	...
                                          	const vector<uint> &posting_block_sizes,
                                          	const vector<uint> &depths, // depth of the first layer
  ...
  for (size_t i = 0; i < depths.size(); ++i) {
    ...
    //The following two cases make sure there is no waste for each greedy-obtained cutoff
