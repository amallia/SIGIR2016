...
public:
	...
	std::vector<int> qidD;
	QueryLogManager(const char* fname, termsMap *l, int& version); //with qid
...
class QueryProcessing {
	...
	pairlists openPLists(const std::vector<std::string>& word_l, termsMap& lex);
	toplayers openTopLayers(const std::vector<std::string>& word_l, termsMap& lex);
...
public:
	...
	/* pair cutoffs 11.8*/
	termsMap load_termpair_cutoffs(const string input);
	/* toplayer cutoffs 11.8*/
	termsMap load_toplayer_cutoffs(const string input);
	/*11.8*/
	ulong ComputeSpaceBudget(const double percent_of_index);
