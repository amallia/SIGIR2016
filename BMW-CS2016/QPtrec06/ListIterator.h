...
public:
	...
	std::vector<listnodelayer> listlayer;
...
public:
	...
	class listlencomparator { // ADDON
		public:
			inline bool operator()(lptr* a, lptr* b) { return a->unpadded_list_length<b->unpadded_list_length;} // sort by listlen (function for sort())
	};
...
// 	// skip blocks until you come to the right one, the original version
...
// 	}
	...
	// skip blocks until you come to the right one, debug version
// 	inline void skipToDidBlockAndDecode(int d) MYNOINLINE	{
// 		cout << "did before skipping: "<< d << endl;
// 		//profiler::getInstance().stepCounter(CONSTS::SKIPS);
// 		// while (d > (int)maxDidPerBlock[block]) {
// 		// 	off += (sizePerBlock[block++]+1);
// 		// 	//profiler::getInstance().stepCounter(CONSTS::SKIPSB);
// 		// }
// 		// while (d > (int)maxDidPerBlock[block]){
// 		// 	off = offsetPerBlock[block+10];
// 		// }
// 		uint oldBlock = block;
// 		// while ((block+10 < numOfBlocks) && (d > (int)maxDidPerBlock[block])){
// 		// 	block+=10;
// 		// }
// 		uint sblock = block;
// 		block++;

// 		uint increase1 = (sizePerBlock[sblock++]+1); 
// 		cout<<"offset increase1 using sizePerBlock: "<< increase1<< endl;
// 		// block++;
// 		// off += increase1;
// 		uint increase2 = 0;
...
// 		// while (d > (int)maxDidPerBlock[block]){
// 		// 	block++;
// 		// 	uint tmp = (sizePerBlock[sblock++]+1);
// 		// 	increase2 += tmp;
// 		// }
// 		while (d > (int)maxDidPerBlock[sblock]){
// 			uint tmp = (sizePerBlock[sblock++]+1);
// 			increase2 += tmp;
// 			// cout<<"offset increase2 step by step total using sizePerBlock: "<< tmp << endl;
// 			// off += tmp;
// 		}
// 		while (d > (int)maxDidPerBlock[block]){
// 			block++;
// 		}
// 		// if (oldBlock > 0){
// 		// 	off += (offsetPerBlock[block]-offsetPerBlock[oldBlock]);
// 		// 	cout<<"off increase: "<< offsetPerBlock[block]-offsetPerBlock[oldBlock]  << endl;
// 		// }
// 	 //    else{
// 	 //    	off += offsetPerBlock[block];
// 	 //    	cout<<"off increase: "<< offsetPerBlock[block] << endl;
// 	 //    }
// 		off += (offsetPerBlock[block-1]-offsetPerBlock[oldBlock]);
// 		cout<<"off increase using offsetPerBlock: "<< offsetPerBlock[block-1]-offsetPerBlock[oldBlock] + increase1  << endl;

// 	    cout<<"offset increase2 total using sizePerBlock: "<< increase2 << endl;
// 	    cout<<"offset increase using sizePerBlock: "<< increase1+increase2 << endl;
// 	    cout<<"Current Block: "<< block << " Current sblock: "<<sblock<<" Old Block: "<< oldBlock << endl;
...
// 		dflag = 1;
// 		fflag = 0;
// 		cout << "did after skipping: "<< did << endl;
...
// 	}
	...
	inline void skipToDidBlockAndDecode(int d) MYNOINLINE	{
		uint oldBlock = block;
		off += (sizePerBlock[block++]+1);
		// while ((block+10 < numOfBlocks) && (d > (int)maxDidPerBlock[block+10])){
		// 	block+=10;
		// }
		// while ((block+5 < numOfBlocks) && (d > (int)maxDidPerBlock[block+5])){
		// 	block+=5;
		// }
		// while ((block+8 < numOfBlocks) && (d > (int)maxDidPerBlock[block+8])){
		// 	block+=8;
		// }
		while ((block+16 < numOfBlocks) && (d > (int)maxDidPerBlock[block+16])){
			block+=16;
		}
		while (d > (int)maxDidPerBlock[block]){
			block++;
...
public:
	...
	inline void sortbylistlen(){
		std::sort(begin(),end(),lptr::listlencomparator());
		for(unsigned int i=0; i<size(); ++i)
			this->operator[](i)->indexInSorted = i;
	}
