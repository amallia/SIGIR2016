...
bool sortlayerlistbyscore(const listnodelayer& a, const listnodelayer& b){
	return (  a.score > b.score );
}

bool sortlayerlistbydid(const listnodelayer& a, const listnodelayer& b){
	return (  a.did > b.did );
}
...
std::vector<lptr> load(const std::string& where=""){
  ...
  while (sqlite3_step(ppStmt) == SQLITE_ROW) {  // For each row returned
	 ...
	 // c.maxDidPerBlock = (unsigned int*) getBlob(ppStmt,2);
	  ...
	  c.offsetPerBlock = c.sizePerBlock;
	  for(uint i = 0; i < c.offsetPerBlock.size(); ++i)
	  	c.offsetPerBlock[i]++;
	  for(uint i = 1; i < c.offsetPerBlock.size(); ++i)
	  	c.offsetPerBlock[i] = c.offsetPerBlock[i] + c.offsetPerBlock[i-1];
	  c.numOfBlocks = c.sizePerBlock.size();

	  // uint st;
	  // for(uint i = 0; i < c.offsetPerBlock.size(); i++){
	  // 	cout << c.sizePerBlock[i] << " " << c.offsetPerBlock[i] << endl;
	  // 	cin >> st;
	  // }
...
void RawIndexList::rankWithBM25(unsigned int* doclen) {
	...
	for(size_t i = 0; i<lengthOfList; ++i)	{
		...
		float score = idf * ( (float)(f * 3)/(float)(f + 2*( 0.25 + 0.75 * (float)(dlen)/(float)(CONSTS::AVGD) ) ) );
		node.score = score;
		scores.push_back(score);
		listlayer.push_back(node);
	}
	sort(listlayer.begin(), listlayer.end(), sortlayerlistbyscore);
...
void RawIndexList::takefirstlayer(){
	float firstlayerlistlength = 0.1*(float)unpadded_list_length;
	unpadded_list_length = (int)firstlayerlistlength;
	lengthOfList = unpadded_list_length;
	vector<listnodelayer> firstlayerlist(listlayer.begin(), listlayer.begin()+(int)firstlayerlistlength);
	sort(firstlayerlist.begin(), firstlayerlist.end(), sortlayerlistbydid);
	doc_ids.clear();
	freq_s.clear();
	for(int i =0; i < firstlayerlist.size(); ++i){
		doc_ids.push_back(firstlayerlist[i].did);
		freq_s.push_back(firstlayerlist[i].freq);
	}
}

void RawIndexList::takesecondlayer(){
	float firstlayerlistlength = 0.1*(float)unpadded_list_length;
	unpadded_list_length = unpadded_list_length - (int)firstlayerlistlength;
	lengthOfList = unpadded_list_length;
	vector<listnodelayer> secondlayerlist(listlayer.begin()+(int)firstlayerlistlength, listlayer.end());
	sort(secondlayerlist.begin(), secondlayerlist.end(), sortlayerlistbydid);
	doc_ids.clear();
	freq_s.clear();
	for(int i =0; i < secondlayerlist.size(); ++i){
		doc_ids.push_back(secondlayerlist[i].did);
		freq_s.push_back(secondlayerlist[i].freq);
	}
}
...
CompressedList::CompressedList(const RawIndexList& rl, const size_t bs) : BS(bs) {
	...
	term = rl.term;
	termId = rl.termId;
	cout << term << " "<<termId<<" "<< lengthOfList << endl;
