...
public:
	...
	// Usage: Given the term and its term_id, return a RawIndexList strucRawIndexList load_raw_list(const std::string& term, size_t wid);ture (vector of scores, dids, freqs) - padded
	RawIndexList load_raw_list(const std::string& term, size_t wid);
	RawIndexList load_raw_list_first_layer(const std::string& term, size_t wid);
	RawIndexList load_raw_list_second_layer(const std::string& term, size_t wid);
