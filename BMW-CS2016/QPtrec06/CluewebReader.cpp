...
RawIndexList CluewebReader::load_raw_list(const std::string& term, size_t wid) {
	...
	Raw_list.original_unpadded_list_length = Raw_list.lengthOfList;
	Raw_list.unpadded_list_length = Raw_list.lengthOfList;

	// Rank all docids
	Raw_list.rankWithBM25(doclen);

	cout << Raw_list.term<<": " << Raw_list.listlayer.size()<<endl;
...
RawIndexList CluewebReader::load_raw_list_first_layer(const std::string& term, size_t wid) {
	// arguments: term, term_id
	BasicList basic_list_Term(term, wid);
	RawIndexList Raw_list(basic_list_Term);
...
RawIndexList CluewebReader::load_raw_list_second_layer(const std::string& term, size_t wid) {
	// arguments: term, term_id
	BasicList basic_list_Term(term, wid);
	RawIndexList Raw_list(basic_list_Term);
...
void CluewebReader::load_baby_index(){
	 ...
	 for (size_t i=0; i<num_terms; i++){
	 			   ...
	 			   CompressedList Clist(Rlist);
	 			   Clist.serializeToDb(CONSTS::trec_output, sql);
	 			   string good_term = term_[i] + "-first";
	 			   string bad_term = term_[i] + "-second";

	 			   if(Rlist.unpadded_list_length > 1000){
		 			   RawIndexList Rlist1 = load_raw_list_first_layer(good_term, i);
		 			   CompressedList Clist1(Rlist1);
		 			   Clist1.serializeToDb(CONSTS::trec_output, sql);
		 			   RawIndexList Rlist2 = load_raw_list_second_layer(bad_term, i); 
		 			   CompressedList Clist2(Rlist2);
		 			   Clist2.serializeToDb(CONSTS::trec_output, sql);
		 			}
	 }
}
